...
class SiegeBoardGame {
    ...
    private function init_hooks() {
        ...
        register_uninstall_hook(__FILE__, array('SiegeBoardGame', 'uninstall'));
        ...
        add_action('wp_ajax_siege_game_action', array($this, 'handle_ajax_request'));
        add_action('wp_ajax_nopriv_siege_game_action', array($this, 'handle_ajax_request'));
    ...
    public function handle_ajax_request() {
        ...
        if (!wp_verify_nonce($_POST['nonce'], 'siege_game_nonce')) {
            wp_die(__('安全验证失败', 'siege-board-game'));
        }
        ...
        switch ($action) {
            case 'make_move':
                $this->handle_make_move();
                break;
            case 'start_game':
                $this->handle_start_game();
                break;
            case 'save_game':
                $this->handle_save_game();
                break;
            case 'load_game':
                $this->handle_load_game();
                break;
            default:
     ...
     * 处理移动
     */
    private function handle_make_move() {
        $move_data = json_decode(stripslashes($_POST['move_data']), true);
        
        // 这里调用游戏逻辑处理移动
        $result = $this->process_game_move($move_data);
     ...
     * 处理开始游戏
     */
    private function handle_start_game() {
        $game_settings = json_decode(stripslashes($_POST['game_settings']), true);
        
        // 初始化新游戏
        $game_state = $this->initialize_new_game($game_settings);
