...
class Siege_Game_Ajax {
    ...
    public static function handle_ajax_request() {
        ...
        switch ($action) {
            ...
            case 'get_game_details':
                self::handle_get_game_details();
                break;
            case 'delete_game':
                self::handle_delete_game();
                break;
            default:
    ...
    private static function handle_clear_cache() {
        if (!current_user_can('manage_options')) {
            wp_send_json_error(__('权限不足', 'siege-board-game'));
        }
        ...
        wp_send_json_success(__('缓存已清除', 'siege-board-game'));
    }
    
    /**
     * 获取游戏详情
     */
    private static function handle_get_game_details() {
        $game_id = intval($_POST['game_id']);
        ...
        if (!$game) {
            wp_send_json_error(__('游戏不存在', 'siege-board-game'));
        }
        ...
        $html .= '<h3>' . sprintf(__('游戏 #%d 详情', 'siege-board-game'), $game_id) . '</h3>';
        $html .= '<table class="form-table">';
        $html .= '<tr><th>' . __('玩家', 'siege-board-game') . '</th><td>' . ($user ? esc_html($user->display_name) : __('未知用户', 'siege-board-game')) . '</td></tr>';
        $html .= '<tr><th>' . __('状态', 'siege-board-game') . '</th><td>' . esc_html($game->status) . '</td></tr>';
        $html .= '<tr><th>' . __('开始时间', 'siege-board-game') . '</th><td>' . esc_html(mysql2date(get_option('date_format') . ' ' . get_option('time_format'), $game->created_at)) . '</td></tr>';
        $html .= '<tr><th>' . __('移动次数', 'siege-board-game') . '</th><td>' . count($moves) . '</td></tr>';
        $html .= '</table>';
        
        if (!empty($moves)) {
            $html .= '<h4>' . __('移动历史', 'siege-board-game') . '</h4>';
            $html .= '<div class="moves-history" style="max-height: 300px; overflow-y: auto;">';
            foreach ($moves as $index => $move) {
                $move_data = json_decode($move->move_data, true);
                $html .= '<div class="move-item">';
                $html .= '<strong>' . sprintf(__('移动 %d', 'siege-board-game'), $index + 1) . '</strong> ';
                $html .= '(' . esc_html($move->player_type) . ') ';
                if (isset($move_data['from']) && isset($move_data['to'])) {
                    $html .= sprintf(__('从 (%d,%d) 到 (%d,%d)', 'siege-board-game'), 
                        $move_data['from']['row'], $move_data['from']['col'],
                        $move_data['to']['row'], $move_data['to']['col']);
                }
                $html .= '</div>';
            }
            $html .= '</div>';
     ...
     * 删除游戏（单个）
     */
    private static function handle_delete_game() {
        $game_id = intval($_POST['game_id']);
